...
class Trace:
    ...
    def __str__(self):
        ...
        result = str(
            f"kind='{self.kind}' parameter='{self.parameter}' input='{self._input}' output='{self._output}' value='{self.value}' result='{self.result}'"
        )
        return result
        ...
        for test_name in intersection:
            ...
            if len(left.traces) != len(right.traces):
                mismatches.add(
                    (
                        test_name,
                        f"trace count are different: {len(left.traces)} != {len(right.traces)}",
                    )
                )
            else:
...
def main(argv):
    ...
    if not os.path.isfile(extracted_tests_trace_file):
        print(
            f"semantic trace file '{extracted_tests_trace_file}' not found. aborting."
        )
        sys.exit(1)
